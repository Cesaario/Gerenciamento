<template>
    <v-container>
        ...
        <v-card v-for="(feedback,i) in feedbacks" :key="i" width=600 class='ma-2'>
            <v-layout v-if='feedback.avaliacao=="negativa"' align-center justify-space-between row fill-height>
                <v-layout  align-center justify-space-between column fill-height>
                    <p class="text-xs-center body-1 ma-2">"{{feedback.comentario}}", {{feedback.autor}}</p>
                </v-layout>    
                <v-btn color='primary' @click.stop.prevent='comentarioResolvido(feedback.id)'>Resolver</v-btn>
            </v-layout>
        </v-card>
    </v-container>
</template>
